...
def send_email(ses_connection, signed_url, email_from, email_subject, email_to,
    ...
    email_body = "Hi,\n\nThe Blink Mobility Platform attempted to " \
                 ...
                 "- Subject: %s \n"\
                 "- Sent: %s\n\n" \
                 "However the email size (including attachments) " \
                 ...
                 "Thanks,\n " \
                 "BlinkMobile Interactive" % (email_subject,
                                              email_date,
                                              signed_url)
...
def main():
    ...
    args.add_argument('--adminEmail', '-a',
                      ...
                      required=False)
    args.add_argument('--accessKey', '-k',
                      help='The AWS Access Key in the Access/Secret Keypair',
                      required=True)
    args.add_argument('--secretKey', '-s',
                      help='The AWS Secret Key in the Access/Secret Keypair',
                      required=True)
    ...
    # We're using an IAM Access / Secret key pair to connect to S3, but still
    # relying on the IAM Role to connect to SES

    if access_key and secret_key:
        conn = boto.connect_s3(access_key, secret_key)
        print 'Connected to S3 using Access Key: %s' % (access_key)
    else:
        conn = boto.connect_s3()
        print 'Connected to S3 using IAM Role'
    ...
    for key in bucket.list(prefix=bucket_prefix):
        ...
        kn = key.name
        key_ascii = kn.encode("utf8", "ignore")
        if key_ascii.endswith('.eml'):
